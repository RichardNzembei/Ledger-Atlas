table "sales" {
  ...
  column "sale_number" {
    null = false
    type = varchar(50)
    comment = "Human-facing reference"
  }
  column "location_id" {
    null = false
    type = binary(16)
  ...
  column "metadata" {
    null = false
    type = json
    comment = "Rule trace, applied discounts, tax breakdown"
  }
  column "completed_at" {
    null = true
    type = timestamp(3)
  ...
  index "idx_sales_tenant_number" {
    unique = true
    columns = [column.tenant_id, column.sale_number]
  }
...
table "payments" {
  ...
  column "reference" {
    null = true
    type = varchar(100)
    comment = "M-Pesa receipt, card auth code, etc."
  }
  column "status" {
    null    = false
