table "assets" {
  ...
  column "asset_tag"      { null = false  type = varchar(64)  comment = "Human-readable asset ID" }
  ...
  column "location_id"    { null = true   type = binary(16)  comment = "Current location" }
  column "assigned_to_user_id" { null = true  type = binary(16) }
  column "acquired_at"    { null = true   type = date }
  column "acquisition_cost" { null = false  type = decimal(14, 4)  default = 0 }
  column "book_value"     { null = false  type = decimal(14, 4)  default = 0 }
  column "depreciation_method" {
    null    = false
    type    = enum("straight_line", "declining_balance", "units_of_production", "none")
    default = "straight_line"
  }
  column "useful_life_months" { null = true  type = int  unsigned = true }
  column "salvage_value"  { null = false  type = decimal(14, 4)  default = 0 }
  column "serial_number"  { null = true   type = varchar(128) }
  ...
  index "idx_assets_tenant_tag"      { unique = true  columns = [column.tenant_id, column.asset_tag] }
