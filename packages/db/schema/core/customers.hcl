table "customers" {
  ...
  column "code"         { null = true   type = varchar(50) }
  column "name"         { null = false  type = varchar(255) }
  column "email"        { null = true   type = varchar(320) }
  column "phone"        { null = true   type = varchar(30) }
  column "segment" {
    null    = false
    type    = enum("retail", "wholesale", "vip", "staff")
    default = "retail"
  }
  column "credit_limit" { null = false  type = decimal(14, 4)  default = 0 }
  column "balance"      { null = false  type = decimal(14, 4)  default = 0  comment = "Running receivables balance" }
  column "address"      { null = true   type = text }
  ...
  index "idx_customers_tenant_email"  { unique = true  columns = [column.tenant_id, column.email] }
  index "idx_customers_tenant_phone"  { columns = [column.tenant_id, column.phone] }
  index "idx_customers_tenant_code"   { unique = true  columns = [column.tenant_id, column.code] }
