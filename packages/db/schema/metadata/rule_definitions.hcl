table "rule_definitions" {
  ...
  column "priority"      { null = false  type = int  default = 100  comment = "Lower = higher priority" }
  column "body"          { null = false  type = json  comment = "Rule definition in the engine native format" }
  column "version"       { null = false  type = int  default = 1 }
  column "is_active"     { null = false  type = boolean  default = false }
  column "authored_by_user_id"            { null = true  type = binary(16) }
  column "authored_from_natural_language" { null = true  type = text }
  column "dry_run_results"                { null = true  type = json }
  column "created_at"    { null = false  type = timestamp(3)  default = sql("CURRENT_TIMESTAMP(3)") }
  column "activated_at"  { null = true   type = timestamp(3) }
...
table "rule_execution_log" {
  ...
  column "tenant_id"    { null = false  type = binary(16) }
  column "rule_id"      { null = false  type = binary(16) }
  column "context_type" { null = false  type = varchar(50)  comment = "sale, stock_check, etc." }
  column "context_id"   { null = false  type = binary(16) }
  column "input"        { null = false  type = json }
  column "output"       { null = false  type = json }
  column "duration_ms"  { null = false  type = int  unsigned = true }
  column "executed_at"  { null = false  type = timestamp(3)  default = sql("CURRENT_TIMESTAMP(3)") }
