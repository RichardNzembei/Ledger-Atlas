table "settings" {
  ...
  column "key"        { null = false  type = varchar(200) }
  column "value"      { null = false  type = json }
...
table "setting_definitions" {
  ...
  column "data_type"    { null = false  type = enum("string", "number", "boolean", "enum", "json") }
  column "default_value"{ null = false  type = json }
  column "allowed_scopes" {
    null    = false
    type    = json
    comment = "Array of: tenant, location, user"
  }
  column "options"      { null = true   type = json  comment = "For enum type: array of {value, label}" }
  column "section"      { null = false  type = varchar(100) }
